...
def main(newbg):
    ...
    def scan_face():
        ...
        Back_button = tk.Button(root, image=Back_icon, command=lambda: start_app())  # Go back to the start screen
        Back_button.place(relx=0.0, rely=1.0, anchor="sw")
    ...
    def flight_seat_selector(Passport_No):
        ...
        # Title label
        start_label = tk.Label(container, text=f"Choose your Flight Seat", font=("Helvetica", 28, "bold"))
        start_label.grid(row=0, column=0, pady=(0, 30))

        # Seat grid frame
        seat_frame = tk.Frame(container, bg= newbg)
    ...
    def Database(Passport_No):
        ...
        label = tk.Label(root, text=f"{Passport_No} Information", font=("Helvetica", 26))
        label.pack(pady=10, anchor='n')
    ...
    def thank_you():
        ...
        content_frame = tk.Frame(border_frame, bg=newbg)  # White inner area
        ...
        settings_icon = ImageTk.PhotoImage(settings_image)
        settings_button = tk.Button(border_frame, image=settings_icon, command=settings_page)
...
main(newbg=root.cget("bg"))  # Start the application
